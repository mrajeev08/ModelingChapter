...
### Coverage example
- From vaccination data from Serengeti
...
## Figure 4: Correlation between bites & rabies suspect cases
```{r}
bites_suspect_TZ <- read.csv("data/monthlybites_suspect_SD_2002_2014.csv")
bites_suspect_MG <- read.csv("data/monthlybites_suspect_Moramanga_Sep2016_Jan2019.csv")
bites_suspect_MG %>%
  filter(month >= 12) -> bites_suspect_MG

bites_suspect_TZ$location <- "TZ"
bites_suspect_MG$location <- "MG"
bites_suspect <- bind_rows(bites_suspect_TZ, bites_suspect_MG)

ggplot(data = bites_suspect, aes(x = total_suspect, y = total_bites, color = location)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  # ylim(c(0, max(bites_suspect$total_bites))) +
  # facet_wrap(~location, scales = "free") +
  ylab("Total incidence of reported bites \n per 100k persons") +
  xlab("Total incidence of rabid exposures \n per 100k persons")
ggsave("figs/bites_suspect.jpeg", device = "jpeg", height = 5, width = 7)  
cor(bites_suspect_TZ$total_suspect, bites_suspect_TZ$total_bites)
nrow(bites_suspect_TZ)
cor(bites_suspect_MG$total_suspect, bites_suspect_MG$total_bites)
nrow(bites_suspect_MG)
