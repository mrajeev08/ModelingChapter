...
library(patchwork)
fig2 <- fdt_plot + ddt_plot
ggsave("figs/fig2.jpeg", fig2, device = "jpeg", width = 16, height = 8)
fig2
```
...
features_D <- ggplot(data = summary, aes(x = reorder(key, key, function(x)-length(x)))) +
  geom_bar(fill = "grey50") +
...
fig3 <- year_A + R0_C + country_B + features_D + plot_layout(ncol = 2, nrow = 2)
ggsave("figs/fig3.jpg", fig3, device = "jpeg", width = 20, height = 15)
fig3
```
...
fig4 <- data_A + data_B + plot_layout(ncol = 2, widths = c(1, 1))
ggsave("figs/fig4.jpeg", plot = fig4, device = "jpeg", height = 6, width = 15)
fig4
...
``` {r extra upset plots}
studies %>%
  mutate_at(c(15:20), function(x) ifelse(x == "N" | is.na(x), 0, 1)) %>%
  select(c(1, 15:20)) -> studies_set
names(studies_set) <- c("Reference", "Underreporting", "Introductions", "Heterogeneity", 
...
names(categories) <- c("Reference", "Estimate parameters", "Identify drivers", 
                       "Explain patterns", 
                       "Study control", "Predict trends")

## Upset plot
pdf("figs/fig5.pdf", onefile = TRUE, width = 10, height = 8)
